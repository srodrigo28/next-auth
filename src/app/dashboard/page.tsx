      ...
      <section className="grid w-full gap-8 rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur sm:p-8 xl:grid-cols-[minmax(0,1.35fr)_360px] xl:p-10 2xl:gap-10 2xl:p-12">
        <div className="space-y-8">
          <div className="space-y-5">
            ...
            <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.3fr)_minmax(360px,.85fr)]">
              <div>
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3 2xl:gap-6">
            {moduleCatalog.map((track) => (
              <Link key={track.slug} href={`/dashboard/${track.slug}`}>
                <article className="group relative h-full overflow-hidden rounded-[1.9rem] border border-white/10 bg-slate-950/45 p-6 shadow-xl transition duration-300 hover:-translate-y-1 hover:border-emerald-400/30 2xl:min-h-[24rem] 2xl:p-8">
                  <div
                    className={`absolute inset-x-0 top-0 h-36 bg-gradient-to-br ${track.accent} opacity-90 blur-2xl transition duration-300 group-hover:opacity-100 2xl:h-44`}
                  />
                  <div className="relative flex h-full flex-col">
                    ...
                    <div className="mt-12 flex-1 2xl:mt-16">
                      <h2 className="text-3xl font-black tracking-tight text-white 2xl:text-[2rem]">
                        {track.title}
                      </h2>
                      <p className="mt-4 text-sm leading-8 text-slate-300 2xl:text-[15px]">
                        {track.description}
                      </p>
                    </div>

                    <div className="mt-8 inline-flex rounded-full bg-emerald-500 px-5 py-3 text-sm font-semibold text-white transition group-hover:bg-emerald-400 2xl:mt-10">
                      Abrir trilha
                    </div>
                  </div>
                </article>
